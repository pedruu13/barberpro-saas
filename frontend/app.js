...
async function confirmBooking() {
  ...
  var barberPixKey = bar.pixKey || state.pixKey;
  if (paymentLabel === 'Pix' && barberPixKey) {
    openPixCheckout(
      'Agendamento: ' + svcNames,
      finalPrice,
      async () => { await executeBookingAPI(bookingPayload, confirmBtn); },
      barberPixKey,
      bar.pixKey ? bar.name : state.shopName
    );
    return;
  }
...
function generatePixPayload(pixKey, amount, merchantName, merchantCity) {
  function formatField(id, value) { return id + String(value.length).padStart(2, '0') + value; }
  var nameStr = (merchantName || 'Barbearia').substring(0, 25).normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase();
  var cityStr = (merchantCity || 'BRASIL').substring(0, 15).normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase();
  var payload = formatField('00', '01') + 
                formatField('26', formatField('00', 'BR.GOV.BCB.PIX') + formatField('01', pixKey)) + 
                formatField('52', '0000') + 
                formatField('53', '986') + 
                (amount ? formatField('54', Number(amount).toFixed(2)) : '') + 
                formatField('58', 'BR') + 
                formatField('59', nameStr) + 
                formatField('60', cityStr) + 
...
function openPixCheckout(title, amount, onConfirm, customPixKey, customMerchantName) {
  const pixKey = customPixKey || state.pixKey;
  if (!pixKey) {
  ...
  const mName = customMerchantName || state.shopName || 'Barbearia';
  $id('pix-title').textContent    = title;
  $id('pix-shop-name').textContent = mName;
  $id('pix-value').textContent    = 'R$ ' + Number(amount).toFixed(2);
  $id('pix-key-display').textContent = generatePixPayload(pixKey, amount, mName, 'BRASIL');
...
async function addBarber() {
  ...
  var pixKey = $id('barber-pix') ? $id('barber-pix').value.trim() : '';
  ...
  const res = await api.post('/admin/barbers', { name, role, emoji, commissionPct, pixKey: pixKey || null });
  state.barbers.push(res);
  closeModal('modal-new-barber');
  renderAdminBarbers();
  ['barber-name','barber-role','barber-emoji','barber-pix'].forEach(function (i) { if($id(i)) $id(i).value = ''; });
...
function openEditBarber(id) {
  ...
  if($id('edit-barber-pix')) $id('edit-barber-pix').value = b.pixKey || '';
...
async function saveEditBarber() {
  ...
  var pixKey = $id('edit-barber-pix') ? $id('edit-barber-pix').value.trim() : '';
  var commissionPct = parseFloat($id('edit-barber-commission') ? $id('edit-barber-commission').value : 0) || 0;
  if (!name) { showToast('⚠️ Informe o nome'); return; }
  const res = await api.put('/admin/barbers/' + id, { name, role, emoji, commissionPct, pixKey: pixKey || null });
  if (res.error) { showToast('❌ ' + res.error); return; }
